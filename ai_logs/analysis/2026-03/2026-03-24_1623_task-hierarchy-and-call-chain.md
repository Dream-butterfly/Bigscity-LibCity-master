...
输入内容：
用户询问 LibCity 中 `traffic_state_pred`、`traffic_speed_pred`、`traffic_flow_pred` 三者的区别与关系，并结合源码进行定位分析。

分析结果：

#### 一、任务层级关系（核心）

- LibCity 实际只注册了一个顶层任务：
  **traffic_state_pred**
...
LibCity 并不是通过 task 区分 speed / flow，而是通过以下三点共同决定：
