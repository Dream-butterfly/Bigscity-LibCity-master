...
脚本 `scripts/run/run_model.py` 用于在 LibCity 中训练和评估单个模型。运行 `scripts/run/run_model.py` 时，必须指定以下三个参数，即**task、dataset 和 model**。例如：

```sh
python scripts/run/run_model.py --task traffic_state_pred --model GRU --dataset METR_LA
```
