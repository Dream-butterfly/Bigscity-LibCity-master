...
class FuzzyRelationalGraphLearner(nn.Module):
    ...
    # ── Semantic Closure (Fuzzy Relational Reasoning) ────────────

    def get_semantic_closure(self, max_hops: int = 3):
        """Compute fuzzy semantic closure S = max(R, R², ..., R^K).

        Em bodies fuzzy relational reasoning via max-min composition:

            (R∘S)[i,j] = max_n min(R[i,n], S[n,j])

        Physical intuition:
          IF node i is strongly related to node k (congestion propagation),
          AND node k is strongly related to node j,
          THEN i and j inherit an inferred transitive relation.

        This transforms the fuzzy relation from a static similarity measure
        into a **fuzzy relational reasoning system** — the closure
        captures higher-order functional connections beyond direct
        membership overlap.
        ...
        Returns:
            S: [N, N] fuzzy semantic closure, values in [0, 1].
               S[i,j] ≥ R[i,j] for all i, j (monotonicity guarantee).
        """
        R = self.forward()  # current fuzzy relation [N, N]
        S = R.clone()
        current = R
        for _ in range(max_hops - 1):
            # Max-min composition: R ∘ current
            current = torch.max(
