"""Spatio-temporal encoder — new_fuzzy_cellattention.

Converts historical traffic sequences [B, Tin, N, Cin] into rich condition
features H [B, Tin, N, D].  Supports standard GCN, fuzzy graph convolution,
and fuzzy cell attention modes.
"""
...
class STEncoderBlock(nn.Module):
    ...
    Sub-layer order:
        temporal self-attn → graph conv → [cell attention] → FFN

    Operates on [B, T, N, D] tensors with Pre-LN residual connections.
    Supports ``GraphConvolution``, ``FuzzyGraphConvolution``,
    and ``FuzzyCellAttention``.
    """
...
class STEncoder(nn.Module):
    ...
    Architecture:
        ...
        → N × STEncoderBlock (temporal attn + graph conv + [cell attn] + FFN)
        → LayerNorm
    """
