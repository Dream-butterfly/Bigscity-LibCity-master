...
class NewFuzzyCellAttention3_Type2(AbstractTrafficStateModel):
    ...
    def _membership_sharpness_loss(self, node_features=None):
        ...
        """
        # Use μ_low (softmax, competitive) — NOT μ_mid.
        # μ_mid = (μ_low+μ_high)/2 is diluted by μ_delta·(1-μ_low),
        # which equalises the simplex → wastes sharpness gradient.
        mu_low, _, _ = self.fuzzy_graph._compute_memberships(node_features)
        # μ_low already a simplex (softmax), no normalisation needed
