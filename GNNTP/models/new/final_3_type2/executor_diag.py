...
class FinalNewExecutor3_Type2_Diag(TrafficStateExecutor):
    """final_3_type2 诊断版 Executor

    继承 FinalNewExecutor3_Type2 的思路，额外在每个 epoch 收集
    model.calculate_loss 中的诊断指标（pred_loss, fir_loss, FOU, |ΔS| 等）
    并输出到日志和 TensorBoard。
    """
    ...
    def train(self, train_dataloader, eval_dataloader):
        ...
        for epoch_idx in range(self._epoch_num, self.epochs):
            ...
            self._unwrap_model().reset_diagnostics()
            losses = self._train_epoch(train_dataloader, epoch_idx, self.loss_func)
            train_loss = float(np.mean(losses))
            diag = self._unwrap_model().get_diagnostics()
            ...
            if self._is_rank0() and (epoch_idx % self.log_every) == 0:
                ...
                if diag:
                    ...
                    for k, v in sorted(diag.items()):
                        fmt = '.6f' if k.endswith('change') else '.4f'
                        label = k
                        if k == 'mu_change':
                            label = '|Δμ|'
                        elif k == 'S_change':
                            label = '|ΔS|'
                        diag_tokens.append(f'{label}:{v:{fmt}}')
                    message += ' | ' + ' '.join(diag_tokens)
                self._logger.info(message)

            # ── Write diagnostics to TensorBoard ──
            if self._is_rank0() and self._writer is not None and diag:
                for k, v in diag.items():
                    self._writer.add_scalar(f'diagnostics/{k}', v, epoch_idx)
