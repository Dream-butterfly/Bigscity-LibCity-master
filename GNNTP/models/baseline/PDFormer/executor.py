...
class PDFormerExecutor(TrafficStateExecutor):
    ...
    关键差异:
      1. 预计算 Laplacian PE (lap_mx) 并传入 model.forward()
      2. 支持 CosineLRScheduler (warmup + cosine decay)
      3. gradient accumulation (grad_accmu_steps)
      4. curriculum learning (task_level 逐步增加)

    Re-migrated from Bigscity-LibCity-master/libcity/executor/pdformer_executor.py
    2026-06-05
    """
        ...
        if self.lr_decay:
            ...
            if self.lr_scheduler_type.lower() == 'multisteplr':
                ...
                lr_scheduler = CosineLRScheduler(
                    self.optimizer, t_initial=self.epochs, lr_min=self.lr_eta_min,
                    decay_rate=self.lr_decay_ratio,
