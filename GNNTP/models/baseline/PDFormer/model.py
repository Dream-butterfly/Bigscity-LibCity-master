...
class STSelfAttention(nn.Module):
    def __init__(
        self, dim, s_attn_size, t_attn_size, geo_num_heads=4, sem_num_heads=2,
        t_num_heads=2, qkv_bias=False, attn_drop=0., proj_drop=0.,
...
class STEncoderBlock(nn.Module):
    def __init__(
        self, dim, s_attn_size, t_attn_size, geo_num_heads=4, sem_num_heads=2,
        t_num_heads=2, mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
        ...
        self.st_attn = STSelfAttention(
            ...
            sem_num_heads=sem_num_heads, t_num_heads=t_num_heads,
    ...
    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        if self.type_ln == 'pre':
            x = x + self.drop_path(self.st_attn(
                self.norm1(x), x_patterns, pattern_keys,
                geo_mask=geo_mask, sem_mask=sem_mask))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        elif self.type_ln == 'post':
            x = self.norm1(x + self.drop_path(self.st_attn(
                x, x_patterns, pattern_keys, geo_mask=geo_mask, sem_mask=sem_mask)))
...
class PDFormer(AbstractTrafficStateModel):
    """
    Paper: PDFormer: Propagation Delay-aware Dynamic Long-range Transformer
           for Traffic Flow Prediction (AAAI 2023)

    Re-migrated from Bigscity-LibCity-master/libcity/model/traffic_flow_prediction/PDFormer.py
    2026-06-05
    """
    ...
    def __init__(self, config, data_feature):
        ...
        if self.max_epoch * self.num_batches * self.world_size < self.step_size * self.output_window:
            self._logger.warning(
                'Parameter `step_size` is too big with {} epochs and '
        ...
        self.pattern_keys = torch.from_numpy(
            data_feature.get('pattern_keys')).float().to(self.device)
        ...
        self.enc_embed_layer = DataEmbedding(
            self.feature_dim - self.ext_dim, self.embed_dim, lape_dim,
            self.adj_mx, drop=drop, add_time_in_day=add_time_in_day,
            add_day_in_week=add_day_in_week, device=self.device,
        )
        ...
        self.encoder_blocks = nn.ModuleList([
            STEncoderBlock(
                ...
                t_attn_size=self.t_attn_size, geo_num_heads=geo_num_heads,
                sem_num_heads=sem_num_heads, t_num_heads=t_num_heads,
                ...
                type_ln=type_ln, output_dim=self.output_dim,
    ...
    def forward(self, batch, lap_mx=None):
        ...
        pattern_key_list = []
        for i in range(self.output_dim):
            x_pattern_list.append(
                self.pattern_embeddings[i](x_patterns[..., i]).unsqueeze(-1))
            pattern_key_list.append(
                self.pattern_embeddings[i](self.pattern_keys[..., i]).unsqueeze(-1))
        ...
        for i, encoder_block in enumerate(self.encoder_blocks):
            enc = encoder_block(enc, x_patterns, pattern_keys,
    ...
    def get_loss_func(self, set_loss):
        if set_loss.lower() not in ['mae', 'mse', 'rmse', 'mape', 'logcosh', 'huber',
                                     'quantile', 'masked_mae', 'masked_mse', 'masked_rmse',
                                     'masked_mape', 'masked_huber', 'r2', 'evar']:
            self._logger.warning(
    ...
    def calculate_loss_without_predict(self, y_true, y_predicted,
        ...
        if self.training:
            ...
            if self.use_curriculum_learning:
                return lf(y_predicted[:, :self.task_level, :, :],
