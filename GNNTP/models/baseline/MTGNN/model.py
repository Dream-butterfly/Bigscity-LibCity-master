...
class MTGNN(AbstractTrafficStateModel):
    """
    Paper: Connecting the Dots: Multivariate Time Series Forecasting
           with Graph Neural Networks (KDD 2020)

    Migrated from Bigscity-LibCity-master/libcity/model/traffic_speed_prediction/MTGNN.py
    """
    ...
    def __init__(self, config, data_feature):
        ...
        if self.max_epoch * self.num_batches < self.step_size * self.output_window:
            self._logger.warning(
                'Parameter `step_size1` is too big with {} epochs and '
        ...
        for i in range(1):
            ...
            for j in range(1, self.layers+1):
                ...
                self.residual_convs.append(nn.Conv2d(
                    in_channels=self.conv_channels, out_channels=self.residual_channels,
                    kernel_size=(1, 1)))
                if self.input_window > self.receptive_field:
                    self.skip_convs.append(nn.Conv2d(
                        in_channels=self.conv_channels, out_channels=self.skip_channels,
                ...
                if self.gcn_true:
                    self.gconv1.append(MixProp(
                        self.conv_channels, self.residual_channels,
                ...
                if self.input_window > self.receptive_field:
                    self.norm.append(LayerNorm(
                        (self.residual_channels, self.num_nodes,
