...
def masked_huber_loss(preds, labels, null_val=0.0, delta=1.0):
    ...
    loss_value = torch.nn.functional.huber_loss(preds, labels, delta=delta, reduction="none")
    loss_value = loss_value * mask
    loss_value = torch.where(torch.isnan(loss_value), torch.zeros_like(loss_value), loss_value)
...
class STGformer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.tod_embedding_dim = config.get("tod_embedding_dim", 12)
        self.dow_embedding_dim = config.get("dow_embedding_dim", 12)
        self.add_time_in_day = config.get("add_time_in_day", config.get("time_of_day", False))
        self.add_day_in_week = config.get("add_day_in_week", config.get("day_of_week", False))
        self.loss_fn = config.get("loss_fn", "masked_huber").lower()
        ...
        adj_mx = self.data_feature.get("adj_mx", None)
        if isinstance(adj_mx, (list, tuple)) and len(adj_mx) > 0:
            adj_mx = adj_mx[0]
        if adj_mx is None:
            support = torch.eye(self.num_nodes, dtype=torch.float32, device=self.device)
        else:
            if hasattr(adj_mx, "toarray"):
                adj_mx = adj_mx.toarray()
            support = torch.tensor(np.asarray(adj_mx), dtype=torch.float32, device=self.device)

        self.network = STGformerCore(
            ...
            adaptive_embedding_dim=config.get("adaptive_embedding_dim", 12),
            ...
            dropout_a=config.get("dropout_a", 0.3),
            kernel_size=kernel_size,
        )
    ...
    def _build_model_input(self, x):
        ...
        if feature_size < self.input_dim:
            raise ValueError(
                f"Input feature_dim({feature_size}) is smaller than configured input_dim({self.input_dim})."
            )
        model_feature_list = [x[..., : self.input_dim]]
        feature_idx = self.input_dim

        tod = torch.zeros_like(x[..., 0])
        if self.add_time_in_day and feature_size > feature_idx:
            tod = torch.clamp(x[..., feature_idx], min=0.0, max=1 - 1e-6)
            feature_idx += 1

        if self.tod_embedding_dim > 0:
            model_feature_list.append(tod.unsqueeze(-1))

        dow = torch.zeros_like(x[..., 0])
        if self.add_day_in_week and feature_size >= feature_idx + 7:
            dow = torch.argmax(x[..., feature_idx: feature_idx + 7], dim=-1).to(x.dtype)
        elif self.add_day_in_week and feature_size > feature_idx:
            dow = x[..., feature_idx]

        if self.dow_embedding_dim > 0:
            model_feature_list.append(dow.unsqueeze(-1))

        return torch.cat(model_feature_list, dim=-1)

    def forward(self, batch):
        x = self._build_model_input(batch["X"])
    ...
    def calculate_loss(self, batch):
        ...
        y_predicted = self.predict(batch)[..., : self.output_dim]
        if self._scaler is not None:
            y_true = self._scaler.inverse_transform(y_true)
            y_predicted = self._scaler.inverse_transform(y_predicted)

        if self.loss_fn == "masked_huber":
            return masked_huber_loss(y_predicted, y_true, null_val=0.0, delta=self.huber_delta)
        if self.loss_fn == "huber":
            return loss.huber_loss(y_predicted, y_true, delta=self.huber_delta)
        if self.loss_fn == "masked_mse":
            return loss.masked_mse_torch(y_predicted, y_true, null_val=0.0)
        if self.loss_fn == "mse":
            return loss.masked_mse_torch(y_predicted, y_true)
        if self.loss_fn == "masked_mae":
