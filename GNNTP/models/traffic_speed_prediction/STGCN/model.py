...
class STGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.graph_conv_type.lower() == 'chebconv':
            ...
            Lk_np = calculate_cheb_poly(laplacian_mx, self.Ks)
            self._logger.info('Chebyshev_polynomial_Lk shape: ' + str(Lk_np.shape))
            self.register_buffer('Lk', torch.FloatTensor(Lk_np))
        elif self.graph_conv_type.lower() == 'gcnconv':
            Lk_np = calculate_first_approx(adj_mx)
            self._logger.info('First_approximation_Lk shape: ' + str(Lk_np.shape))
            self.register_buffer('Lk', torch.FloatTensor(Lk_np))
