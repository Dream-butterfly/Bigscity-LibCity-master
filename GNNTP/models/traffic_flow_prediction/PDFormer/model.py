...
class PDFormer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.type_short_path == "dist":
            ...
            far_mask = torch.zeros(self.num_nodes, self.num_nodes).to(self.device)
            far_mask[sd_mx < self.far_mask_delta] = 1
            self.register_buffer('far_mask', far_mask.bool())
        else:
            sh_mx = sh_mx.T
            geo_mask = torch.zeros(self.num_nodes, self.num_nodes).to(self.device)
            geo_mask[sh_mx >= self.far_mask_delta] = 1
            self.register_buffer('geo_mask', geo_mask.bool())
            sem_mask = torch.ones(self.num_nodes, self.num_nodes).to(self.device)
            dtw_sorted = self.dtw_matrix.argsort(axis=1)[:, :self.dtw_delta]
            for i in range(sem_mask.shape[0]):
                sem_mask[i][dtw_sorted[i]] = 0
            self.register_buffer('sem_mask', sem_mask.bool())

        self.register_buffer('pattern_keys', torch.from_numpy(data_feature.get('pattern_keys')).float().to(self.device))
